...
add_action( 'after_setup_theme', 'medigreen_register_menus' );
add_action( 'wp_enqueue_scripts', 'medigreen_enqueue_scripts_styles' );
add_action( 'tgmpa_register', 'medigreen_register_plugins' );
add_action( 'wp_enqueue_scripts', 'medigreen_load_fonts' );
add_action( 'admin_init', 'medigreen_theme_add_editor_styles' );
add_action( 'admin_enqueue_scripts', 'medigreen_load_fonts' );
add_action( 'admin_enqueue_scripts', 'medigreen_load_admin_style' );

add_action( 'widgets_init', 'medigreen_widget_area' );

add_filter( 'bt_bb_color_scheme_arr', 'medigreen_color_schemes' );

add_filter( 'body_class', 'medigreen_body_class' );

add_filter( 'tiny_mce_before_init', 'medigreen_editor_dynamic_styles' );
 ...
 */
if ( ! function_exists( 'medigreen_register_menus' ) ) {
	function medigreen_register_menus() {
		register_nav_menus( array (
			'primary' => esc_html__( 'Primary Menu', 'medigreen' ),
			'footer'  => esc_html__( 'Footer Menu', 'medigreen' )
		));
	}
}
 ...
 */
if ( ! function_exists( 'medigreen_enqueue_scripts_styles' ) ) {
	function medigreen_enqueue_scripts_styles() {

		wp_enqueue_script( 'medigreen-custom-script', get_parent_theme_file_uri( 'js/misc.js' ), array( 'jquery' ), '', true );
		...
		// custom theme css
		wp_enqueue_style( 'medigreen-style', get_parent_theme_file_uri( 'style.css' ), array(), false, 'screen' );
		wp_enqueue_style( 'medigreen-print', get_parent_theme_file_uri( 'print.css' ), array(), false, 'print' );

		// external js
		wp_enqueue_script( 'fancySelect', get_parent_theme_file_uri( 'framework/js/fancySelect.js' ), array( 'jquery' ), '', true );

		// custom theme js
		wp_enqueue_script( 'medigreen-header', get_parent_theme_file_uri( 'framework/js/header.misc.js' ), array( 'jquery' ), '', true );
		wp_enqueue_script( 'medigreen-misc', get_parent_theme_file_uri( 'framework/js/misc.js' ), array( 'jquery' ), '', true );
		
		wp_add_inline_script( 'medigreen-header', boldthemes_set_global_uri(), 'before' );
		...
		if ( file_exists( get_parent_theme_file_path( 'css-override.php' ) ) && $boldthemes_add_override_css ) {
			...
			wp_add_inline_style( 'medigreen-style', $css_override );
		}
		
		if ( file_exists( get_parent_theme_file_path( 'icons.php' ) ) ) {
			require_once( get_parent_theme_file_path( 'icons.php' ) );
			wp_add_inline_style( 'medigreen-style', $icons );
		}

		if ( boldthemes_get_option( 'custom_js' ) != '' ) {
			wp_add_inline_script( 'medigreen-misc', boldthemes_get_option( 'custom_js' ) );
		}	
 ...
 */
if ( ! function_exists( 'medigreen_register_plugins' ) ) {
	function medigreen_register_plugins() {

		$plugins = array(
	 
			array(
				'name'               => esc_html__( 'Medigreen', 'medigreen' ), // The plugin name.
				'slug'               => 'medigreen', // The plugin slug (typically the folder name).
				'source'             => get_parent_theme_file_path( 'plugins/medigreen.zip' ), // The plugin source.
			...
			array(
				'name'               => esc_html__( 'Cost Calculator', 'medigreen' ), // The plugin name.
			...
			array(
				'name'               => esc_html__( 'Bold Builder', 'medigreen' ), // The plugin name.
			...
			array(
				'name'               => esc_html__( 'AIKO: Instant Plugins AI Developer Lite', 'medigreen' ), // The plugin name.
			...
			array(
				'name'               => esc_html__( 'BoldThemes WordPress Importer', 'medigreen' ), // The plugin name.
			...
			array(
				'name'               => esc_html__( 'Meta Box', 'medigreen' ), // The plugin name.
			...
			array(
				'name'               => esc_html__( 'Contact Form 7', 'medigreen' ), // The plugin name.
			...
			array(
				'name'               => esc_html__( 'Lightweight Sidebar Manager', 'medigreen' ), // The plugin name.
		...
		$config = array(
			...
			'strings'      => array(
				...
				'installing'                      => esc_html__( 'Installing Plugin: %s', 'medigreen' ), // %s = plugin name.
				'oops'                            => esc_html__( 'Something went wrong with the plugin API.', 'medigreen' ),
				'notice_can_install_required'     => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.', 'medigreen' ), // %1$s = plugin name(s).
				'notice_can_install_recommended'  => _n_noop( 'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.', 'medigreen' ), // %1$s = plugin name(s).
				'notice_cannot_install'           => _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.', 'medigreen' ), // %1$s = plugin name(s).
				'notice_can_activate_required'    => _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', 'medigreen' ), // %1$s = plugin name(s).
				'notice_can_activate_recommended' => _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', 'medigreen' ), // %1$s = plugin name(s).
				'notice_cannot_activate'          => _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.', 'medigreen' ), // %1$s = plugin name(s).
				'notice_ask_to_update'            => _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.', 'medigreen' ), // %1$s = plugin name(s).
				'notice_cannot_update'            => _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.', 'medigreen' ), // %1$s = plugin name(s).
				'install_link'                    => _n_noop( 'Begin installing plugin', 'Begin installing plugins', 'medigreen' ),
				'activate_link'                   => _n_noop( 'Begin activating plugin', 'Begin activating plugins', 'medigreen' ),
				...
				'complete'                        => esc_html__( 'All plugins installed and activated successfully. %s', 'medigreen' ), // %s = dashboard link.
 ...
 */
if ( ! function_exists( 'medigreen_load_fonts' ) ) {
	function medigreen_load_fonts() {
		$body_font = urldecode( boldthemes_get_option( 'body_font' ) );
		$heading_font = urldecode( boldthemes_get_option( 'heading_font' ) );
		$menu_font = urldecode( boldthemes_get_option( 'menu_font' ) );
		$heading_subtitle_font = urldecode( boldthemes_get_option( 'heading_subtitle_font' ) );
		...
		if ( $body_font != '' ) {
			 ...
			 */
			if ( 'off' !== _x( 'on', 'Lato font: on or off', 'medigreen' ) ) {
		...
		if ( count( $font_families ) > 0 ) {
			...
			wp_enqueue_style( 'medigreen-fonts', $font_url, array(), '1.0.0' );
			add_editor_style( $font_url );
		}
	}
}

if ( ! function_exists( 'medigreen_load_admin_style' ) ) {
	function medigreen_load_admin_style() {
		if ( function_exists( 'boldthemes_csscrush_file' ) ) {
		...
		wp_enqueue_style( 'medigreen-admin-style', get_parent_theme_file_uri( 'admin-style.css' ) );
		require_once( get_parent_theme_file_path( 'admin-style.php' ) );
		wp_add_inline_style( 'medigreen-admin-style', $admin_style );
	}
}
 ...
 * TinyMCE style
 */
if ( ! function_exists( 'medigreen_theme_add_editor_styles' ) ) {
	function medigreen_theme_add_editor_styles() {
 ...
 */
if ( ! function_exists( 'medigreen_editor_dynamic_styles' ) ) {
	function medigreen_editor_dynamic_styles( $mceInit ) {
...
if ( ! function_exists( 'medigreen_body_class' ) ) {
	function medigreen_body_class( $extra_class ) {
		...
		if ( boldthemes_get_option( 'page_width' ) == 'boxed_1690' ) {
			$extra_class[] = 'btBoxedPage1690';
		}
		return $extra_class;
	}
}

if ( ! function_exists( 'medigreen_widget_area' ) ) {
	function medigreen_widget_area() {
		if ( class_exists( 'woocommerce' ) ) {
			register_sidebar( array (
				'name' 			=> esc_html__( 'Shop Sidebar', 'medigreen' ),
				'id' 			=> 'bt_shop_sidebar',
