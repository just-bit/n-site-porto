 ...
 * Version: 3.0.4
 * Author: BoldThemes
 * Author URI: https://codecanyon.net/user/boldthemes
 */
...
function btcc_enqueue() {
	wp_enqueue_script( 'btcc_purify', plugins_url( 'purify.min.js', __FILE__ ), array( 'jquery' ), BT_CC_VERSION );
	wp_enqueue_script( 'btcc_dd', plugins_url( 'jquery.dd.js', __FILE__ ), array( 'jquery' ), BT_CC_VERSION );
	wp_enqueue_script( 'btcc_main', plugins_url( 'cc.main.js', __FILE__  ), array( 'jquery' ), BT_CC_VERSION );
	$js = '';
	...
	wp_enqueue_style( 'bt_cc_style', plugins_url( 'style.css', __FILE__ ), array(), BT_CC_VERSION );
	
	wp_register_style( 'btcc_style_inline', false );
	wp_enqueue_style( 'btcc_style_inline' );	
	$custom_css = '.product a.add_to_cart_button.bt_cc_disabled { opacity: .5 !important; pointer-events: none !important; } .bt_cc_wc_quote > div { position: relative; left: 1ch; } .bt_cc_wc_quote > div:before { content: \'•\'; position: absolute; left: -1ch; }';
...
function btcc_admin_enqueue() {
	...
	wp_enqueue_script( 'btcc_custom_fields', plugins_url( 'bt_cc_custom_fields.js', __FILE__  ), array( 'jquery' ), BT_CC_VERSION );
	wp_enqueue_style( 'btcc_admin_style', plugins_url( 'admin_style.css', __FILE__ ), array(), BT_CC_VERSION );
	wp_enqueue_style( 'bt-bb-light-override', plugins_url( 'bt_bbl_override.css', __FILE__ ), array(), BT_CC_VERSION );
}
...
add_action( 'init', 'btcc_load_textdomain' );
...
class bt_cost_calculator {
	...
	static function handle_shortcode( $atts, $content ) {
		extract( shortcode_atts( array(
			...
			'paypal_currency'     => '',
			'wc_product_id'       => '',
			'wc_quote_text'       => esc_html__( 'Quote', 'bt-cost-calculator' ),
			'email_gdpr'          => '',
			'email_gdpr_text'     => '',
			'email_gdpr_not_text' => '',
			'email_rtl'           => '',
		...
		wp_enqueue_script( 'bt_touch-punch_js', plugins_url( 'jquery.ui.touch-punch.min.js', __FILE__ ), array( 'jquery-ui-slider' ), BT_CC_VERSION );
		...
		if ( $wc_quote_text == '' ) $wc_quote_text = esc_html__( 'Quote', 'bt-cost-calculator' );
		...
		if ( class_exists( 'WooCommerce' ) ) {
			...
			if ( is_product() ) { // single product
				...
				$single_product_page = 'yes';
				$el_class .= ' btQuoteBookingWooCommerceSingleProduct';
			} else { // product shortcode (not single product page)
				$single_product_page = 'no';
				$el_class .= ' btQuoteBookingWithWooCommerceProduct';
				if ( $wc_product_id != '' ) {
					global $product;
		...
		$output = '<div class="btQuoteBooking ' . $el_class . ' ' . $css_class . '" ' . $style_attr . ' data-admin_email="' .  $admin_email. '" data-cf7_id="' .  $cf7_id. '" data-email_client="' . $email_client . '" data-email_gdpr="' . $email_gdpr . '" data-email_gdpr_text="' . $email_gdpr_text . '" data-email_gdpr_not_text="' . $email_gdpr_not_text . '" data-email_rtl="' . $email_rtl . '" data-currency="' . $currency . '" data-currency_after="' . $currency_after . '" data-currency_space="' . $currency_space . '" data-url_confirmation="' . $url_confirmation . '" data-url_ajax="' . admin_url( 'admin-ajax.php' ) . '" data-subject="' . $subject . '" data-email_header="' . $email_header . '" data-email_footer="' . $email_footer . '" data-date_text="' . $date_text . '" data-time_text="' . $time_text . '" data-message_please_wait="' . esc_html__( 'Please wait...', 'bt-cost-calculator' ) . '" data-message_success="'. esc_html__( 'Thank you, we will contact you soon!', 'bt-cost-calculator' ) .'" data-message_error="' . esc_html__( 'Error! Please try again later.', 'bt-cost-calculator' ) . '" data-message_mandatory="' .  esc_html__( 'Please fill out all required fields.', 'bt-cost-calculator' ) . '" data-rec_secret_key="' . $rec_secret_key . '" data-total_text="' . $total_text . '" data-total_format="' . $total_format . '" data-total_decimals="' . $total_decimals . '" data-date_format="' . $date_format . '" data-bt_cc_nonce="' . wp_create_nonce( 'bt_cc_nonce' ) . '" data-hide_total="' . $hide_total . '" data-hide_subtotal="' . $hide_subtotal . '" data-wc_product_id="' . $wc_product_id . '" data-wc_quote_text="' . $wc_quote_text . '" data-wc_single_product="' . $single_product_page . '" data-wc_product_price="' . $wc_product_price . '"><div class="btQuoteBookingWrap">';
			...
			if ( $wc_product_id != '' && class_exists( 'WooCommerce' ) && ! is_product() ) {
				if ( $product ) {
					$output .= do_shortcode( '[add_to_cart id="' . $wc_product_id . '" style=""]' );
				}
			}
...
class Cost_Proxy {
	...
	public function css_init() { 
			...
			if ( $this->accent_color != '' ) {
				...
				wp_register_style( 'btcc_style_inline_accent_color', false );
				wp_enqueue_style( 'btcc_style_inline_accent_color' );

				wp_add_inline_style( 'btcc_style_inline_accent_color', $custom_css );
			}
...
class CostDD_Proxy {
	...
	public $title;
	public $img_height;
	public $initial_index;
	public $no_empty;
	function __construct( $dd_id, $items_arr, $title, $img_height, $initial_index, $no_empty ) {
		$this->dd_id = $dd_id;
		...
		<script>
			...
			document.addEventListener('readystatechange', function() {
				if ( ! bt_cc_<?php echo $this->dd_id; ?>_init_finished && typeof(jQuery) !== 'undefined' && ( document.readyState === 'interactive' || document.readyState === 'complete' ) ) {
					...
					var ddData = [<?php
						if ( $this->no_empty !== 'yes' ) {
							$arr = explode( ';', $this->items_arr[0] );
							if ( is_numeric( $arr[1] ) ) {
								echo '{ text:\'' . $this->title . '\', value:\'0\' },';
							} else {
								echo '{ text:\'' . $this->title . '\', value:\'\' },';
							}
						}
						...
						foreach ( $this->items_arr as $item ) {
							...
							if ( trim( $item ) != '' ) {
								...
								echo '{ text:\'' . $arr[0] . '\', value:\'' . sanitize_text_field( $arr[1] ) . '\', description:\'' . sanitize_text_field( $arr[2] ) . '\', image:\'' . $arr[3] . '\' }';
								if ( $i < count( $this->items_arr ) ) {
									echo ',';
								}
							}
						}
...
class bt_cc_item {
	...
	static function handle_shortcode( $atts, $content ) {
		...
		$number_min = sanitize_text_field( $number_min );
		$number_max = sanitize_text_field( $number_max );
		$number_step = sanitize_text_field( $number_step );
		$initial_value = sanitize_text_field( $initial_value );
		$no_empty = sanitize_text_field( $no_empty );
		$raw_value_only = sanitize_text_field( $raw_value_only );
		...
		if ( $type == 'text' ) {
			...
			if ( $number_min != '' ) {
				$min_html = ' min="' . $number_min . '"';
			}
			if ( $number_max != '' ) {
				$max_html = ' max="' . $number_max . '"';
			}
			if ( $number_step != '' ) {
				$step_html = ' step="' . $number_step . '"';
			}
			$input = '<input type="number" class="btQuoteText btQuoteElement"' . $phtml . $min_html . $max_html . $step_html . ' data-condition="' . $condition . '" data-price="' . $price . '" value="' . $initial_value . '" data-initial-value="' . $initial_value  . '" data-raw-value-only="' . $raw_value_only . '"/>';
			...
			$input = '<div id="' . $dd_id . '" class="btQuoteSelect btContactField btDropDown btQuoteElement" data-condition="' . $condition . '" data-initial-value="' . $initial_value  . '" data-raw-value-only="' . $raw_value_only . '"></div>';
			
			$proxy = new CostDD_Proxy( $dd_id, $items_arr, esc_html__( 'Select...', 'bt-cost-calculator' ), $img_height, $initial_value, $no_empty );
			add_action( 'wp_footer', array( $proxy, 'js_init' ), 20 );			
			...
			$input = '<div class="btQuoteSlider btQuoteElement" data-value="' . $initial_value . '"  data-initial-value="' . $initial_value  . '" data-min="' . $arr[0] . '" data-max="' . $arr[1] . '" data-step="' . $arr[2] . '" data-price="' . $price . '" data-offset="' . $offset . '" data-condition="' . $condition . '" data-raw-value-only="' . $raw_value_only . '"></div><span class="btQuoteSliderValue">' . $initial_value . '</span>';
			...
			$input = '<div class="btQuoteSwitch btQuoteElement' . $class_on . '" data-off="' . $arr[0] . '" data-on="' . $arr[1] . '" data-condition="' . $condition . '" data-initial-value="' . $initial_value  . '" data-raw-value-only="' . $raw_value_only . '"><div class="btQuoteSwitchInner"></div></div>';
...
function bt_quote_map_sc() {
	...
	$bt_quote_params = array(
		...
		array( 'param_name' => 'hide_subtotal', 'type' => 'checkbox', 'value' => array( esc_html__( 'Yes', 'bt-cost-calculator' ) => 'yes' ), 'heading' => esc_html__( 'Hide Subtotal(s)', 'bt-cost-calculator' ) ),
		...
		array( 'param_name' => 'date_text', 'type' => 'textfield', 'heading' => esc_html__( 'Placeholder - Date Picker', 'bt-cost-calculator' ), 'value' => esc_html__( 'Preferred Service Date', 'bt-cost-calculator' ), 'group' => esc_html__( 'Contact Form', 'bt-cost-calculator' ) ),
		...
		array( 'param_name' => 'time_text', 'type' => 'textfield', 'heading' => esc_html__( 'Placeholder - Time Picker', 'bt-cost-calculator' ), 'value' => esc_html__( 'Preferred Service Time', 'bt-cost-calculator' ), 'group' => esc_html__( 'Contact Form', 'bt-cost-calculator' ) ),
		...
		array( 'param_name' => 'wc_product_id', 'type' => 'textfield', 'heading' => esc_html__( 'WooCommerce Product ID', 'bt-cost-calculator' ), 'group' => esc_html__( 'WooCommerce', 'bt-cost-calculator' ), 'description' => __( 'Read more in our <a href="https://documentation.bold-themes.com/cost-calculator/using-cost-calculator/#woocommerce" target="_blank">documentation</a>.', 'bt-cost-calculator' ) ),
		array( 'param_name' => 'wc_quote_text', 'type' => 'textfield', 'heading' => esc_html__( 'Quote Title in Cart/Order', 'bt-cost-calculator' ), 'description' => esc_html__( 'Default is "Quote". You can try "Custom Price", "Offer", etc.', 'bt-cost-calculator' ), 'group' => esc_html__( 'WooCommerce', 'bt-cost-calculator' ) ),
	...
	$bt_cc_group_params = array(
		...
		array( 'param_name' => 'eval', 'type' => 'textarea_object', 'heading' => esc_html__( 'Pseudo-JS Code', 'bt-cost-calculator' ), 'description' => __( 'For detailed explaination on how to use it go to <a target="_blank" href="https://documentation.bold-themes.com/cost-calculator/cost-calculator-group/">documentation</a>.', 'bt-cost-calculator' ) ),
    ...
    $bt_cc_text_params = array(
		...
		array( 
			...
			'holder' => 'div', 
			'description' => esc_html__( 'Label for the Input field - visible to user.', 'bt-cost-calculator' )
		),
		array( 
		...
		array( 'param_name' => 'description', 'type' => 'textfield', 'heading' => esc_html__( 'Description', 'bt-cost-calculator' ), 'holder' => 'div', 'description' => esc_html__( 'Description - visible to user.', 'bt-cost-calculator' ) ),
        array( 'param_name' => 'value', 'type' => 'textfield', 'heading' => esc_html__( 'Unit Value *required', 'bt-cost-calculator' ), 'description' => esc_html__( 'The number from input field will by multiplied by this value. Unit value is not visible to user.', 'bt-cost-calculator' ) ),
		array( 'param_name' => 'initial_value', 'type' => 'textfield', 'heading' => esc_html__( 'Initial Value', 'bt-cost-calculator' ), 'description' => esc_html__( 'Initial value for input field (default is 0).', 'bt-cost-calculator' ) , 'preview' => true),
		array( 'param_name' => 'min', 'type' => 'textfield', 'heading' => esc_html__( 'Min', 'bt-cost-calculator' ), 'description' => esc_html__( 'Minimum value allowed.', 'bt-cost-calculator' ) , 'preview' => true),
		array( 'param_name' => 'max', 'type' => 'textfield', 'heading' => esc_html__( 'Max', 'bt-cost-calculator' ), 'description' => esc_html__( 'Maximum value allowed.', 'bt-cost-calculator' ) , 'preview' => true),
		array( 'param_name' => 'step', 'type' => 'textfield', 'heading' => esc_html__( 'Step', 'bt-cost-calculator' ), 'description' => esc_html__( 'How much value can change.', 'bt-cost-calculator' ) , 'preview' => true),
		array( 'param_name' => 'raw_value_only', 'type' => 'checkbox', 'value' => array( esc_html__( 'Yes', 'bt-cost-calculator' ) => 'yes' ), 'heading' => esc_html__( 'Show Raw Value Only', 'bt-cost-calculator' ), 'description' => __( 'Only raw item value will be shown in quote (in email, log, etc.). Read more in our <a href="https://documentation.bold-themes.com/cost-calculator/using-cost-calculator/" target="_blank">documentation</a>.', 'bt-cost-calculator' ) ),
		array( 
		'param_name' => 'condition', 
		'type' => 'textarea_object', 
		'heading' => esc_html__( 'Conditional Settings', 'bt-cost-calculator' ), 
		'description' =>
			__( 'For detailed explaination on how to use it go to <a target="_blank" href="https://documentation.bold-themes.com/cost-calculator/cost-calculator-conditions/">documentation</a>.', 'bt-cost-calculator' )
		),
		array( 'param_name' => 'item_el_id', 'type' => 'textfield', 'heading' => esc_html__( 'Custom ID Attribute', 'bt-cost-calculator' ), 'group' => esc_html__( 'Design', 'bt-cost-calculator' ), 'preview' => true ),
		array( 'param_name' => 'item_el_class', 'type' => 'textfield', 'heading' => esc_html__( 'Extra Class Name(s)', 'bt-cost-calculator' ), 'group' => esc_html__( 'Design', 'bt-cost-calculator' ), 'preview' => true ),
		array( 'param_name' => 'item_el_style', 'type' => 'textfield', 'heading' => esc_html__( 'Inline Style', 'bt-cost-calculator' ), 'group' => esc_html__( 'Design', 'bt-cost-calculator' ), 'preview' => true )
    ...
    $bt_cc_select_params = array(
		...
		array( 'param_name' => 'name', 'type' => 'textfield', 'heading' => esc_html__( 'Label', 'bt-cost-calculator' ), 'holder' => 'div', 'description' => esc_html__( 'Label for select list - visible to user.', 'bt-cost-calculator' ) ),
		array( 'param_name' => 'description', 'type' => 'textfield', 'heading' => esc_html__( 'Description', 'bt-cost-calculator' ), 'holder' => 'div', 'description' => esc_html__( 'Description - visible to user.', 'bt-cost-calculator' ) ),
		array( 'param_name' => 'value', 'type' => 'textarea', 'heading' => esc_html__( 'List of Dropdown Options *required', 'bt-cost-calculator' ), 'description' => __( 'List of options visible to user as dropdown list. For detailed explaination go to <a target="_blank" href="https://documentation.bold-themes.com/cost-calculator/cost-calculator-items/#select">documentation</a>.', 'bt-cost-calculator' ) ),
		array( 'param_name' => 'no_empty', 'type' => 'checkbox', 'value' => array( esc_html__( 'Hide blank option at start of list', 'bt-cost-calculator' ) => 'yes' ), 'heading' => esc_html__( 'Blank Selection', 'bt-cost-calculator' ), 'description' => esc_html__( 'When selected, the list will start with the first added item. When unselected, the list will start with a blank option.', 'bt-cost-calculator' ) ),
		array( 'param_name' => 'initial_value', 'type' => 'textfield', 'heading' => esc_html__( 'Initially Selected Option', 'bt-cost-calculator' ), 'description' => esc_html__( 'Enter the number of the option to be selected by default (0 = first item, 1 = second item, etc). Note: If blank option is enabled, it counts as the first item (0).', 'bt-cost-calculator' ) , 'preview' => true),
		array( 'param_name' => 'raw_value_only', 'type' => 'checkbox', 'value' => array( esc_html__( 'Yes', 'bt-cost-calculator' ) => 'yes' ), 'heading' => esc_html__( 'Show Raw Value Only', 'bt-cost-calculator' ), 'description' => __( 'Only raw item value will be shown in quote (in email, log, etc.). Read more in our <a href="https://documentation.bold-themes.com/cost-calculator/using-cost-calculator/" target="_blank">documentation</a>.', 'bt-cost-calculator' ) ),
		...
		array( 'param_name' => 'condition', 'type' => 'textarea_object', 'heading' => esc_html__( 'Conditional Settings', 'bt-cost-calculator' ), 'description' => __( 'For detailed explaination on how to use it go to <a target="_blank" href="https://documentation.bold-themes.com/cost-calculator/cost-calculator-conditions/">documentation</a>.', 'bt-cost-calculator' ) ),
		array( 'param_name' => 'item_el_id', 'type' => 'textfield', 'heading' => esc_html__( 'Custom ID Attribute', 'bt-cost-calculator' ), 'group' => esc_html__( 'Design', 'bt-cost-calculator' ), 'preview' => true ),
		array( 'param_name' => 'item_el_class', 'type' => 'textfield', 'heading' => esc_html__( 'Extra Class Name(s)', 'bt-cost-calculator' ), 'group' => esc_html__( 'Design', 'bt-cost-calculator' ), 'preview' => true ),
		array( 'param_name' => 'item_el_style', 'type' => 'textfield', 'heading' => esc_html__( 'Inline Style', 'bt-cost-calculator' ), 'group' => esc_html__( 'Design', 'bt-cost-calculator' ), 'preview' => true )
	);
    ...
    $bt_cc_slider_params = array(
		...
		array( 'param_name' => 'name', 'type' => 'textfield', 'heading' => esc_html__( 'Label', 'bt-cost-calculator' ), 'holder' => 'div', 'description' => esc_html__( 'Label for slider bar - visible to user.', 'bt-cost-calculator' ) ),
		array( 'param_name' => 'description', 'type' => 'textfield', 'heading' => esc_html__( 'Description', 'bt-cost-calculator' ), 'holder' => 'div', 'description' => esc_html__( 'Description - visible to user.', 'bt-cost-calculator' ) ),
		...
		array( 'param_name' => 'value_unit', 'type' => 'textfield', 'heading' => esc_html__( 'Unit Value *required', 'bt-cost-calculator' ), 'description' => esc_html__( 'The number selected with slider handle will by multiplied by this value. Unit value is not visible to user.', 'bt-cost-calculator' ), 'preview' => true ),
		array( 'param_name' => 'value_offset', 'type' => 'textfield', 'heading' => esc_html__( 'Offset', 'bt-cost-calculator' ), 'description' => esc_html__( 'Slider has price offset property which allows you to introduce fixed cost regardless of the volume. This is not visible to user.', 'bt-cost-calculator' ), 'preview' => true ),
		array( 'param_name' => 'initial_value', 'type' => 'textfield', 'heading' => esc_html__( 'Initial Value', 'bt-cost-calculator' ), 'description' => esc_html__( 'Slider will be initially set to this value.', 'bt-cost-calculator' ) , 'preview' => true ),
		array( 'param_name' => 'raw_value_only', 'type' => 'checkbox', 'value' => array( esc_html__( 'Yes', 'bt-cost-calculator' ) => 'yes' ), 'heading' => esc_html__( 'Show Raw Value Only', 'bt-cost-calculator' ), 'description' => __( 'Only raw item value will be shown in quote (in email, log, etc.). Read more in our <a href="https://documentation.bold-themes.com/cost-calculator/using-cost-calculator/" target="_blank">documentation</a>.', 'bt-cost-calculator' ) ),
		array( 'param_name' => 'condition', 'type' => 'textarea_object', 'heading' => esc_html__( 'Conditional Settings', 'bt-cost-calculator' ), 'description' => __( 'For detailed explaination on how to use it go to <a target="_blank" href="https://documentation.bold-themes.com/cost-calculator/cost-calculator-conditions/">documentation</a>.', 'bt-cost-calculator' ) ),
		array( 'param_name' => 'item_el_id', 'type' => 'textfield', 'heading' => esc_html__( 'Custom ID Attribute', 'bt-cost-calculator' ), 'group' => esc_html__( 'Design', 'bt-cost-calculator' ), 'preview' => true ),
		array( 'param_name' => 'item_el_class', 'type' => 'textfield', 'heading' => esc_html__( 'Extra Class Name(s)', 'bt-cost-calculator' ), 'group' => esc_html__( 'Design', 'bt-cost-calculator' ), 'preview' => true ),
    ...
    $bt_cc_switch_params = array(
		...
		array( 'param_name' => 'name', 'type' => 'textfield', 'heading' => esc_html__( 'Label', 'bt-cost-calculator' ), 'holder' => 'div', 'description' => esc_html__( 'Label for toggle switch - visible to user.', 'bt-cost-calculator' ) ),
		array( 'param_name' => 'description', 'type' => 'textfield', 'heading' => esc_html__( 'Description', 'bt-cost-calculator' ), 'holder' => 'div', 'description' => esc_html__( 'Description - visible to user.', 'bt-cost-calculator' ) ),
		array( 'param_name' => 'value_off', 'type' => 'textfield', 'heading' => esc_html__( 'Value Off *required', 'bt-cost-calculator' ), 'description' => esc_html__( 'Value when switch is OFF.', 'bt-cost-calculator' ), 'preview' => true ),
		array( 'param_name' => 'value_on', 'type' => 'textfield', 'heading' => esc_html__( 'Value On *required', 'bt-cost-calculator' ), 'description' => esc_html__( 'Value when switch is ON.', 'bt-cost-calculator' ), 'preview' => true ),
		array( 'param_name' => 'initial_value', 'type' => 'checkbox', 'value' => array( esc_html__( 'Yes', 'bt-cost-calculator' ) => 'yes' ), 'heading' => esc_html__( 'Switch to ON initially', 'bt-cost-calculator' ) ),
		array( 'param_name' => 'raw_value_only', 'type' => 'checkbox', 'value' => array( esc_html__( 'Yes', 'bt-cost-calculator' ) => 'yes' ), 'heading' => esc_html__( 'Show Raw Value Only', 'bt-cost-calculator' ), 'description' => __( 'Only raw item value will be shown in quote (in email, log, etc.). Read more in our <a href="https://documentation.bold-themes.com/cost-calculator/using-cost-calculator/" target="_blank">documentation</a>.', 'bt-cost-calculator' ) ),
		array( 'param_name' => 'condition', 'type' => 'textarea_object', 'heading' => esc_html__( 'Conditional Settings', 'bt-cost-calculator' ), 'description' => __( 'For detailed explaination on how to use it go to <a target="_blank" href="https://documentation.bold-themes.com/cost-calculator/cost-calculator-conditions/">documentation</a>.', 'bt-cost-calculator' ) ),
		array( 'param_name' => 'item_el_id', 'type' => 'textfield', 'heading' => esc_html__( 'Custom ID Attribute', 'bt-cost-calculator' ), 'group' => esc_html__( 'Design', 'bt-cost-calculator' ), 'preview' => true ),
		array( 'param_name' => 'item_el_class', 'type' => 'textfield', 'heading' => esc_html__( 'Extra Class Name(s)', 'bt-cost-calculator' ), 'group' => esc_html__( 'Design', 'bt-cost-calculator' ), 'preview' => true ),
	...
	BTCC_Root::$builder->map( 'bt_cc_switch', array( 'name' => esc_html__( 'Cost Calculator Toggle Switch', 'bt-cost-calculator' ), 'description' => esc_html__( 'Cost calculator switch checkbox', 'bt-cost-calculator' ), 'icon' => 'bt_bb_icon_bt_cc_switch', 'show_settings_on_create' => true,
...
add_action( 'init', 'bt_quote_map_sc' );
